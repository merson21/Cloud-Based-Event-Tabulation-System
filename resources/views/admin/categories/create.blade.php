    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('percentage') ? 'is-invalid' : '' }}" name="percentage" id="percentage" required>
                    ...
                    @foreach(App\Models\Category::PERCENTAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('percentage', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('partipants') ? 'is-invalid' : '' }}" type="number" name="partipants" id="partipants" value="{{ old('partipants', '') }}" step="1">

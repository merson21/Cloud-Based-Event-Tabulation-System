    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.candidates.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('partylist') ? 'is-invalid' : '' }}" name="partylist_id" id="partylist_id" disabled>
                    <option value="" selected>Please select</option>
                    {{-- @foreach($partylists as $id => $entry)
                        <option value="{{ $id }}" {{ old('partylist_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach --}}
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('position') ? 'is-invalid' : '' }}" name="position_id" id="position_id" disabled required>
                    <option value="" selected>Please select</option>
            ...
            <div class="form-group">
                <label>{{ trans('cruds.candidate.fields.gender') }}</label>
...
<script>
    $(document).ready(function () {
        $('#organization_id').change(function (e) {
            ...
            $.ajax({
                url: "{{ route('admin.combobox.position') }}",
                data: {
                ...
                success: function (data) {
                    $position.html('<option value="" selected>Please select</option>');
                        $.each(data, function (id, value) {
                            $position.append('<option value="' + id + '">' + value + '</option>');
                        });
            ...
            var $partylist = $('#partylist_id');
            $.ajax({
                url: "{{ route('admin.combobox.partylist') }}",
                data: {
                        organization_id: $(this).val()
                    },
                success: function (data) {
                    $partylist.html('<option value="" selected>Please select</option>');
                        $.each(data, function (id, value) {
                            $partylist.append('<option value="' + id + '">' + value + '</option>');
                        });

                    $('#partylist_id').attr('disabled', false);
                }
            });

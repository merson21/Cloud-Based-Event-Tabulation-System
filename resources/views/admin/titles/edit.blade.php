    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.titles.update", [$title->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Title::STATUS_2_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status_2') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_2_{{ $key }}" name="status_2" value="{{ $key }}" {{ old('status_2', $title->status_2) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $title->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $title->slug) }}" required>
                @if($errors->has('slug'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text" name="location" id="location" value="{{ old('location', $title->location) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $title->date) }}">
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('score_min') ? 'is-invalid' : '' }}" type="number" name="score_min" id="score_min" value="{{ old('score_min', $title->score_min) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('score_max') ? 'is-invalid' : '' }}" type="number" name="score_max" id="score_max" value="{{ old('score_max', $title->score_max) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('basetype') ? 'is-invalid' : '' }}" name="basetype" id="basetype" required>
                    ...
                    @foreach(App\Models\Title::BASETYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('basetype', $title->basetype) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
